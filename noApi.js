...
async function handleRedirect(request, env, shortCode) {
  ...
  return Response.redirect(url, parseInt(redirectType, 10) || 302);
}
...
async function addLink(env, data) {
  ...
  const expireDays = parseInt(data.expireDays, 10) || 0;
...
async function batchImport(env, data) {
  try {
    ...
    for (const item of list) {
      ...
      const expireDays = item.expireDays ? parseInt(item.expireDays, 10) : 0;
...
function renderHome(env) {
  ...
  const html =
    ...
    "async function createShort(){var url=targetInput.value.trim();var code=codeInput.value.trim();if(!url){showToast('请输入目标 URL');return;}if(!code){code=await generateUniqueCode(8);codeInput.value=code;}if(code.length>MAX_LEN){showToast('短码长度不能超过 '+MAX_LEN);return;}if(codeExists){showToast('短码已存在，请更换或随机生成');return;}var token=localStorage.getItem('adminToken');if(!token){token=await requestAdminToken();if(!token)return;}try{var res=await fetch('/admin',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+token},body:JSON.stringify({action:'add',shortCode:code,url:url,expireDays:90,redirectType:302,password:''})});if(!res.ok){var t=await res.text();showToast(t);return;}var full=window.location.origin+'/'+code;shortUrlSpan.textContent=full;resultBox.classList.remove('hidden');}catch(e){showToast('创建失败');}}" +
